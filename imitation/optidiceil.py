...
class OptiDICEIL(nn.Module):
    def __init__(self, policy, env, best_policy=None,
                 ...
                 n_train=1, n_valid=1, train_lambda=True, lamb_scale=1.0, use_target_nu=False, 
                 weight_norm=True, weighted_replay_sampling=False, add_absorbing_state=False):
        
        torch.manual_seed(seed)
        np.random.seed(seed)
        random.seed(seed)
        
        super(OptiDICEIL, self).__init__()
        ...
        self.e_network = nn.Sequential(
            nn.Linear(self.obs_dim + self.action_dim, self.e_hidden_size, device=self.device),
            # nn.LeakyReLU(0.2, inplace=True),
            nn.ReLU(inplace=True),
            nn.Linear(self.e_hidden_size, 1, device=self.device)
        )
    ...
    def train(self, total_iteration=1e6, eval_freq=1000, batch_size=1024):
        ...
        for num in range(0, int(total_iteration)+1):
            for _ in range(self.inner_steps):
                ...
                nu_loss1 = - self.alpha * f_w_nu.mean()
                nu_loss2 = (w_nu * (e_target - self.lamb_scale * self.lamb)).mean()
                nu_loss3 = self.lamb_scale * (self.lamb + self.lamb ** 2 /2 )
                
                nu_loss = nu_loss0 + nu_loss1 + nu_loss2 + nu_loss3
            ...
            reward = - ((actions - self.policy(obs).mean)**2).mean(dim=-1).detach()
            nu_s = self.nu_network(obs).reshape(-1)
            nu_s_prime =  self.nu_network(next_obs).reshape(-1)
            ...
            if self.weight_norm:
                ...
                policy_loss = (normalized_w_e.detach() * ((actions - self.policy(obs).mean)**2).mean(dim=-1)).mean()
            else:
                policy_loss = (w_e.detach() * ((actions - self.policy(obs).mean)**2).mean(dim=-1)).mean()
            
            # policy_loss = (w_e.detach() * reward).mean()
            ...
            if (num) % eval_freq == 0:
                ...
                reward_valid = - ((actions_valid - self.policy(obs_valid).mean)**2).mean(dim=-1)
                nu_s_valid = self.nu_network(obs_valid).reshape(-1)
                nu_s_prime_valid =  self.nu_network(next_obs_valid).reshape(-1)

                e_target_valid = reward_valid + (1 - terminals_valid) * self.gamma * nu_s_prime_valid - nu_s_valid
                w_e_valid = self.w_fn ((e_target_valid - self.lamb_scale * self.lamb)/ self.alpha)
                
                # reward_valid = - ((actions_valid - self.policy(obs_valid).mean)**2).mean(dim=-1)
                policy_loss_valid = (w_e_valid.detach() * (-reward_valid)).mean()
                
                mse_loss_train = (-reward).mean()
                mse_loss_valid = (-reward_valid).mean()
                ...
                if self.wandb:
                    log_dict = {
                            ...
                            'nu/loss2':             nu_loss2.item(),
                            'nu/loss3':             nu_loss3.item(),
